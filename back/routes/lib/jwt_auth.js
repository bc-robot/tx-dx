...
function authJwt(ctx,secret) {
    return new Promise(function(resolve, reject) {
        ...
        // console.log(ctx.cookies,ctx.cookies.get('jwt_access_token'), 'this is cookies');
        if (jwt_access_token) {
            try {
                jwt.verify(jwt_access_token, secret, function(err, decoded) {
                    if (err) {
                        ...
                        if (decoded.exp >= (decoded.exp.toString().length == 10 ? Date.now(): Math.floor(Date.now()/1000))) {
